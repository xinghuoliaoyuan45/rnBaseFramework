...
export default class CountDownTime extends Component {
	...
	static propTypes = {
		isStart:PropTypes.bool,// true为起计时，false/undefined 为倒计时
	...
	getData(){
		...
		if(!this.props.isStart){
			date =this.getDateData(this.props.time);
		}else{
			date = this.getDateData(0);
		}
		if (date) {
	...
	start(){
		if(!this.props.isStart){
			this.getLeftTime(this.props.time);
		}else{
			this.getBeginTime(0);
		}
	}
		...
		this.interval = setInterval(()=> {
			...
			if (date) {
				...
				if(time==this.props.time){
					// console.log("已经到达截止时间",this.props.time);
					this.stop();
				}
				time++;
			} else {
	...
	render() {
		...
		return (
			<View style={this.props.containerStyle}>
				...
				{/*<Text style={this.props.secondColonStyle}>{this.props.segSuffix}</Text>*/}
			</View>
